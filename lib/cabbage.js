...
/**
 * Authenticate and create a websocket for the given room.
 *
 * @param {Number} roomId The room to connect to.
 * @param [String] fkey The chat fkey.
 * @return {Promise} Promise for the websocket.
 */
function createWebSocket (roomId, fkey) {
	return request({
		method: 'POST',
		...
		return new WebSocket(websocketAddress + '?l=0', { origin: 'http://chat.stackoverflow.com' });
	});
}
...
CabbageBot.prototype.listen = function CabbageBot_listen (roomId) {
	...
	// chat auth request
	return createWebSocket(roomId, this.fkey).then(function (ws) {
		this.ws = ws;
		var self = this;
 ...
 *
 * @param {Number} roomId The room to join.
 * @return {Promise} Promise to join.
 */
CabbageBot.prototype.join = function CabbageBot_join (roomId) {
	if (this.rooms.indexOf(roomId) !== -1) {
		return Promise.reject(new Error('Already joined room ' + roomId));
	}
	this.rooms.push(roomId);
